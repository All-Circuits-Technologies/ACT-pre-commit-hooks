...
###  - python3 -m pipx [install|upgrade] reuse
        ...
        do
            ...
            reuse annotate \
                ...
                echo "Reuse issue detected. `
                     `Forcing dotlicense for ${git_file}."
                # Fallback to a forced dotlicence.
                # Kept despite addition of --fallback-dot-license in reuse 3.0.0
                # in order to overcome rare cases where file extension is known
                # but reuse crashed when parsing it (fails to parse input file).
                reuse annotate \
                    --force-dot-license \
                    --merge-copyrights\
                    --license "${ACT_LICENSE_NAME}" \
